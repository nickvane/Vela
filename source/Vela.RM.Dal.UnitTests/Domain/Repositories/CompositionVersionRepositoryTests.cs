...
namespace Vela.RM.Dal.UnitTests.Domain.Repositories
{
	...
	public class WhenUsingCompositionVersionRepository
	{
		...
		public void Setup()
		{
			...
			_collection = new List<CompositionVersion>
			              	{
			              		new CompositionVersion
			              			{
			              				VersionedComposition = new VersionedComposition
			              				                       	{
			              				                       		Uid = new HierObjectId(_id1)
			              				                       	},
			              				Version = new OriginalVersion<Composition>
			              				          	{
			              		...
			              		new CompositionVersion
			              			{
			              				...
			              				Version = new ImportedVersion<Composition>
			              				          	{
		...
		private IQueryable<CompositionVersion> _collection;
		...
		public void GetAllVersions()
		{
			...
			var repository = new CompositionVersionRepository(session, _collection);
		...
		public void GetVersionAndHasVersion()
		{
			...
			Expect.Call(session.Load<CompositionVersion>(Id6)).Return(new CompositionVersion());
			_mocks.ReplayAll();
		...
		public void IsOriginalVersionTrue()
		{
			...
			Expect.Call(session.Load<CompositionVersion>(Id6)).Return(new CompositionVersion(){Version = new OriginalVersion<Composition>()});
			_mocks.ReplayAll();
