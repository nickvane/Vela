...
{
	[TestFixture]
	public class VersionFromTrunkTests
	{
			...
			_versions = new List<CompositionVersion>
			            	{
			            		...
			            		new CompositionVersion {Version = new OriginalVersion<Composition> {Uid = new ObjectVersionId(Id4)}}
			            	}.AsQueryable();
		}
		...
		{
			var result = _versions.Where(new VersionsFromTrunk().IsSatisfiedBy()).ToList();
